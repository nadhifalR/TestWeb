  ...
  const columns = useMemo<ColumnDef<any>[]>(() => [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className={`w-full flex items-center gap-3 p-3 rounded-lg transition-colors text-left ${isSelected ? 'bg-blue-600/10 border-blue-500/20 border' : 'hover:bg-blue-500/5'}`}>
    ...
    <header className="sticky top-0 z-30 h-16 theme-bg bg-opacity-80 backdrop-blur-md border-b theme-border px-6 flex items-center justify-between">
      <div className="max-w-md w-full relative" ref={searchRef}>
        ...
        {showResults && flatResults.length > 0 && (
          <div className="absolute top-full mt-2 left-0 right-0 theme-card border theme-border shadow-2xl rounded-lg overflow-hidden z-50 animate-in fade-in slide-in-from-top-2">
